    ...
    searchAPI(e) {
      ...
      axios
        .get(
          ...
          }/symptoms-search?page=1&size=50&search=` + this.searchValue
        )
        .then((res) => {
...
<template>
  <input v-model="inputValue" @change="searchAPI($event)" placeholder="Type here..." class="textField" />
  ...
  <div v-else>
    <ol id="highlightString">
      <li v-for="string in response" :key="string">
        <p v-for="item in getHighlightString(string)" :key="item" class="inlinePrint">
          <span v-if="item.toLowerCase() === this.searchValue.toLowerCase()"><b>{{ item }}</b></span>
